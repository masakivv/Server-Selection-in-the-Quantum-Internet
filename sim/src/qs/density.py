...
class SimulationConfig:
    ...
    # Timing control
    timing_mode: str = "factor"  # "factor" or "absolute"
    gate_time_ns: Optional[float] = None  # used when timing_mode == "absolute"
...
DEFAULT_THETA = 0.2297  # rad 単位の理論角度
...
def single_angle_metrics(theta: float, config: SimulationConfig, seed: int) -> Tuple[float, Dict[str, Any]]:
    """Run ZZ/XX circuits once and collect the combined energy metrics."""

    kwargs = config.as_kwargs()
    zz_cost, zz_time, zz_rho, zz_exps = ZZ_cost_density(
        angle=theta, flag=0, base_seed=seed, **kwargs
    )
    xx_cost, xx_time, xx_rho, xx_exps = XX_cost_density(
        angle=theta, flag=1, base_seed=seed, **kwargs
    )
...
def run_density(
    *,
    ...
    # ensure we don't mutate the module-level default instance
    cfg = replace(cfg)
    ...
    if seed_sweep is not None:
        ...
        for sweep_seed in seeds:
            _, metrics = single_angle_metrics(theta_value, cfg, sweep_seed)
    ...
    energy, metrics = single_angle_metrics(theta_value, cfg, seed)
